...
cycle:
    movzx EDX, DX
    mov SI, word ptr [EBX+EDI*(type Date_pack)]
    movzx ESI, SI
    push ESI
    push EDX
    call Less
...
Rec_to_Struc proc
    ...
    mov EBX, [EBP+12]
    shr AX, D
    mov (Date_unpack ptr [EBX]).D, AL
    mov EAX, [EBP+8]
    and AX, mask M
    shr AX, M
    mov (Date_unpack ptr [EBX]).M, AL
    mov EAX, [EBP+8]
    and AX, mask Y
    mov (Date_unpack ptr [EBX]).Y, AL
...
L1: cmp ECX, N
    jE ffin
    movzx EAX, Arr_of_Rec[ECX*type Date_pack]
    push EAX
    call Out_Rec
    outchar ' '
    inc ECX
...
L2:
    cmp AL, 1
    jNE noo
    movzx ESI, word ptr Arr_of_Rec[ECX*(type Date_pack)]
    movzx EDI, word ptr Arr_of_Rec[EDX*(type Date_pack)]
    push EDI
    push ESI
    call Less
    inc ECX
...
; решение:
 ...
 L4:
    ...
    movzx EBX, Arr_of_Rec[ECX*(type Date_pack)]
    push EAX
    push EBX
    call Rec_to_Struc
    inc ECX
